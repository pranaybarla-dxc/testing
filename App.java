...
public class App {
	...
	public void getAllUsers() throws NamingException {
		...
		while (users.hasMore()) {
			...
			addUserToGroup(attr.get("cn").toString(),"Administrators");
	...
	public void addUser() {
		...
		// user details
		attributes.put("sn", "Roggers");
		try {
			connection.createSubcontext("cn=Tom,ou=users,ou=system", attributes);
	...
	{
		try {
			connection.destroySubcontext("cn=Jim,ou=users,ou=system");
			System.out.println("success");
		} catch (NamingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
